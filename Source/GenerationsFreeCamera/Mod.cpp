...
std::unordered_set<void*> players;
...
HOOK(void*, __fastcall, CameraDestructor, 0x10FB630, Camera* This)
{
...
void __cdecl addPlayer(void* player)
{
    players.insert(player);
}

uint32_t addPlayerTrampolineReturnAddress = 0xE796AE;
uint32_t addPlayerTrampolineFunctionAddress = 0xD601F0;

void __declspec(naked) addPlayerTrampoline()
{
    __asm
    {
        push eax
        ...
        call[addPlayerTrampolineFunctionAddress]
        jmp[addPlayerTrampolineReturnAddress]
    }
}

void __cdecl destructPlayer(void* player)
{
    players.erase(player);
}

uint32_t destructPlayerTrampolineReturnAddress = 0xE78F59;
uint32_t destructPlayerTrampolineFunctionAddress = 0x4A0D70;

void __declspec(naked) destructPlayerTrampoline()
{
    __asm
    {
        push edi
        ...
        call[destructPlayerTrampolineFunctionAddress]
        jmp[destructPlayerTrampolineReturnAddress]
    }
}
...
HOOK(void*, __fastcall, UpdateDirectorNormal, 0x11B60B0, void* This, void* Edx, void* context, float elapsedTime)
{
    if (pauseGame)
    {
        ...
        addUpdateCommand(context, elapsedTime, "4");
        ...
        addRenderCommand(context, elapsedTime, "4");
...
HOOK(void*, __fastcall, UpdateApplication, 0xE7BED0, void* This, void* Edx, float elapsedTime, uint8_t a3)
{
    ...
    if (config->teleportPlayer.isTapped(inputState))
    {
...
extern "C" void __declspec(dllexport) Init()
{
    ...
    WRITE_JUMP(0xE796A9, addPlayerTrampoline);
    WRITE_JUMP(0xE78F54, destructPlayerTrampoline);
    ...
    {
        // Activation Manager
        WRITE_MEMORY(0xEA5A90, uint8_t, 0x00);
        WRITE_MEMORY(0xEA6E24, uint8_t, 0x00);
    }
